...
function LeaderBoard() {
  ...
  useGSAP(() => {
    ...
    sectionsRef.current.forEach((section, index) => {
      if (section) {
        gsap.fromTo(section, { opacity: 0, y: 30 }, { opacity: 1, y: 0, duration: 0.7, delay: index * 0.2 })
      }
    })
    ...
    buttonRefs.current.forEach((button, index) => {
      if (button) {
        gsap.fromTo(button, { scale: 0 }, { scale: 1, duration: 0.6, delay: 0.5 + index * 0.1, ease: "back.out(1.7)" })
      }
    })
  }, [])

  const categories = [
    { key: 'all', title: 'Overall Leaderboard' },
    { key: 'people', title: 'People Quiz Leaderboard' },
    { key: 'process', title: 'Process Quiz Leaderboard' },
    { key: 'business', title: 'Business Environment Leaderboard' }
  ]
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <Header>
        ...
        <Link to="/howworks" className="hover:text-accent">How the Quiz Works</Link>
        <Link to="https://rofenac.github.io/blog/" className="hover:text-accent">My DevLog</Link>
      </Header>

      <section className="flex-grow py-24">
        <div className="container mx-auto px-5">
          <h2 ref={titleRef} className="text-4xl font-bold text-center mb-10">Leaderboard</h2>
          ...
          {categories.map(({ key, title }, index) => (
            <div key={key} ref={el => sectionsRef.current[index] = el} className="mt-24 mb-24">
              ...
              <div className="flex justify-center mt-4">
                <button ref={el => buttonRefs.current[index] = el} className="btn btn-warning" onClick={() => clearLeaderboard(key)}>
                  Clear {title}
                </button>
              </div>
            </div>
